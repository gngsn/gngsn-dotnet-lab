...
public class Player
{
    ...
    public Player(int gameAreaLeft, int gameAreaTop, int gameAreaWidth, int gameAreaHeight)
    {
        ...
        // Start player at bottom center, very close to the bottom
        X = gameAreaWidth / 2;
        Y = gameAreaHeight - 2; // Very close to bottom of game area
    ...
    public void Update()
    {
        ...
        for (int i = Bullets.Count - 1; i >= 0; i--)
        {
            ...
            // Remove bullets that are out of bounds (only remove when they go beyond the screen)
            if (Bullets[i].Y < 0)
            {
                Bullets.RemoveAt(i);
            }
