...
public static class GameManager
{
    ...
    private static void InitializeEnemies()
    {
        ...
        // Position enemies very close to the top of the game area
        // Top row (5 enemies): ><, oo, ><, oo, ><
        int enemyY = 0; // Almost at the very top
        int startX = 5;
        int spacing = 4;
        ...
        enemyY = 2; // Still very close to top but below first row
        startX = 7;
        spacing = 6;
        ...
        if ((DateTime.Now - lastEnemyMove).TotalMilliseconds >= 500)
        {
            ...
            if (shouldMoveDown)
            {
                ...
                foreach (var enemy in enemies.Where(e => e.IsAlive))
                {
                    ...
                    if (enemy.Y >= gameAreaHeight - 4) // Leave some space before actual bottom
                    {
                        playerWon = false;
                        gameRunning = false;
                        return;
